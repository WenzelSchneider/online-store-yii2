   ...
   public function actionParse($idf)
{
        ...
        if (stristr($value, 'import'.$idf) == true && stristr($value, 'import_files') == false )
        {
                ...
                ->Товары->Товар as $item)
            {
                ...
                if ($item->Картинка)
                {
                    ...
                    $imag = str_replace('https://ural-dekor.ru/frontend','', $lil->image_link);
                    if(file_exists("../".$imag)){
            ...
            if(stristr($value, 'orders') === FALSE) {
                ...
                foreach($xml->Документ AS $doc){
                    ...
                    if($maxvell < $num){
                            ...
                            if($doc->ЗначенияРеквизитов->ЗначениеРеквизита[2]->Значение == 'true'){
                                    ...
                                    if(count($query) > 0){
                                       // $ordersMain->sync_furn_return = 1; 
                                            $ordersMain->an_comment = $doc->ЗначенияРеквизитов->ЗначениеРеквизита[3]->Значение;
                                            $ordersMain->sync_furn_return = array_search($doc->ЗначенияРеквизитов->ЗначениеРеквизита[3]->Значение, $state);
                                    ...
                                    foreach($doc->ЗначенияРеквизитов->ЗначениеРеквизита as $rec){
                                        if($rec->Наименование == "ПроцентОплаты"){
                                            $ordersMain->balance_furn = $rec->Значение;
                                ...
                                if ($order_state != array_search($doc->ЗначенияРеквизитов->ЗначениеРеквизита[3]->Значение, $state) && in_array(array_search($doc->ЗначенияРеквизитов->ЗначениеРеквизита[3]->Значение, $state), $alarm)) { 
                                    ...
                                    ->setTo($client['email'])
                                    ->setSubject('Статус заказа №'.$num.' изменен')
                        ...
                        echo $client_order_id . ', ' . $doc->ЗначенияРеквизитов->ЗначениеРеквизита[3]->Значение;
    ...
    public function actionCheck(){
        ...
        $goods_count = Goods::find()
            ->Where(['>','updated_datetime', date("Y-m-d 0:0:0", time()-(60*60*24))])
            ->andWhere(['is_hide' => 0])
            ->count();
        
        $price_count = Price::find()
            ->where(['>','value', 0])
            ->orWhere(['>','up_date', date("Y-m-d 0:0:0", time()-(60*60*24))])
            ->count();
        
        $inos = OrdersMain::find()
            ->where('(sync_plit = 0 AND updated_datetime > "' . date("Y-m-d H:i:s", time()-(60*60*24)) . '" AND updated_datetime < "' . date("Y-m-d H:i:s", time()-(60*60*0.5)) . '" AND sync_furn = 0 AND test_order LIKE "no")')
            ->all();
  
        $outos = OrdersMain::find()
            ->where('(sync_plit_return = 0 AND updated_datetime > "' . date("Y-m-d H:i:s", time()-(60*60*24)) . '" AND updated_datetime < "' . date("Y-m-d H:i:s", time()-(60*60*0.5)) . '" AND sync_furn_return = 0 AND test_order LIKE "no")')
            ->all();
